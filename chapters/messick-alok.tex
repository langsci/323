...
\abstract{\cite{wurmbrand17} shows that \emph{that}-less complements can embed the ellipsis construction known as stripping in English. In Hindi, it is possible to embed stripping even in the presence of the complementizer like element \emph{ki}. We argue that the crucial difference between English and Hindi is the position in the structure the complementizer resides. The analysis of Hindi stripping also sheds light on negative stripping and alternative questions.}
...
\z 
\cite{kush16} also reports similar variation in such structures, which he refers to as single remnant gapping. For a subset of his consultants, the examples in \REF{maex11} and \REF{maex12} are acceptable.
...
\ea \label{maex19}
    ...
    \z 
\z 
It is unclear whether such movement is obligatory, however. As we see in \REF{maex20}, the wh-elements can also remain in-situ without issue, and in some cases, sound more natural than their counterparts in \REF{maex19}.
...
\ex \label{maex30}
    ...
    \glt `We called you, not them.'
\z 
In the literature on negative stripping, there has been two proposals about the structure of negation \citep{merchant03,wurmbrand17,dendikken18}. Under one view, it is argued that negation in negative stripping is the result of a high sentential negation \REF{maex31}. The other view argues instead that such structures involve constituent negation \REF{maex32}.
...
\ea \label{maex35}
    ...
    \z 
\z 
We assume the structure below in \REF{maex36} for the second disjunct in alternative questions. Once again, \emph{ki} will act as a subordination marker, there is movement of the remnant to a focus projection followed by clausal ellipsis. This analysis hence allows us to keep a uniform syntax for \emph{ki} (i.e., it always takes a clause complement) and also accounts for the lack of P-omission in Hindi.
...
%[Disj$'$ 
%    [{Disj\\(yaa)}] [SubP
%        [ki] [FocP [PP$_i$ [Mohan ke liye, roof]] [Foc$'$
%            [Foc] [FinP[\dots \emph{t}$_i$ dots, roof]]
%    ] ] ] ]
...
[yaa+ki$_i$,name=ki]
[SubP
[\emph{t}$_i$,name=trace]
[FocP [PP$_j$[Mohan ke liye, roof]] [Foc$'$ [Foc]
[\sout{Fin} [\dots \emph{t}$_j$ \dots, roof]]
]
]
]
]
\draw[->] (trace) to[out=south west,in=south] (ki);
\end{forest}
\end{comment}
