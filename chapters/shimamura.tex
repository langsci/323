...
\z
Therefore, we need to dissociate the intention holder from the actual doer; the simplest case is such that the former and the latter are identical, hence the case of \textit{I will leave} in \REF{shimamu6}. Then, I assume that \textit{yoo} has its person parameter fixed to the 1st person, denoting the speaker's volitional attitude. Structurally, we have:

\ea\label{shimamu8}
\Tree [.TP [.ModP [.$v$P DP [.$v'$ \qroof{\textit{leave}}.VP $v$ ] ] [.Mod {\textit{yoo}\newline\textsc{[person: 1]}} ] ] T ] 
\z
In \REF{shimamu8}, the attitude holder of \textit{yoo} is expressed in terms of the person feature on the modal head, and the actual doer (agent) is merged to Spec-$v$P. Therefore, the agent can be anything, be it 1st person, 2nd person, 3rd person, singular or plural.
...
\ea\label{shimamu13} \hspace{-1.5mm}*{\gll Karera-wa [ Hirosi-ni [ $e$ otagai-o naguri-a-oo-to ] omow ]-ase-ta.\\
...
\end{xlist}
\z
To capture the differences under discussion, I propose the following two structures:

\ea\label{shimamu22}
\begin{xlist}
\ex\label{shimamu22a} \Tree [.VP [.RepP [.TP [.ModP [.$v$P \qroof{\textit{pro}}.DP [.$v'$ \qroof{\ldots}.VP $v$ ] ] [.Mod {\textit{yoo}\newline\textsc{[person: 1]}} ] ] T ] [.Rep \textit{to}$_{\includegraphics[width=3mm]{figures/monster}}$ ] ][.V \textit{decide}/\textit{think} ] ]
\ex\label{shimamu22b} \Tree [.VP [.RepP [.ModP \qroof{\ldots}.VP [.Mod {\textit{yoo}\newline\textsc{[person: 1]}} ] ]  [.Rep \textit{to}$_{\includegraphics[width=3mm]{figures/monster}}$ ] ][.V \textit{try}/\textit{do} ] ]
\end{xlist}
\z 
In \REF{shimamu22a}, the embedded clause has $v$P as well as TP, whereas \REF{shimamu22b} lacks them. Unlike \REF{shimamu4a}, I assume that TP is present. Note that in Japanese, the finite future can be expressed by the present form if a given verb is eventive, and when embedded, it is interpreted relative to the matrix reference time (Taro's now in \REF{shimamu23}). Observe:
...
Then, the structure in \REF{shimamu22a} can be considered to be more ``biclausal'' than that in \REF{shimamu22b}, which is supported by the fact about the Negative Concord Items (NCI); the combination of wh-pronouns and \textit{-mo} `also' yields NCIs such as \textit{dare-mo} (who-also) `anyone', which requires the presence of a negation as in \REF{shimamu25}, and unlike negative polarity items, NCIs need a given negation to be in the same clause where they are located, as shown in \REF{shimamu26}.
...
At this point, the two structures in \REF{shimamu22} give us another interesting prediction: that is, the accusative case that is assigned to the embedded object stems from the embedded verb in \REF{shimamu22a} and the matrix verb in \REF{shimamu22b} since an accusative case, by assumption, is assigned by (transitive) $v$, so that long passive should be possible in \REF{shimamu22b}, but not in \REF{shimamu22a}. This prediction turns out to be true as follows:\footnote{Some of my language consultants did not like \REF{shimamu28b}, but they still saw the clear contrast between \REF{shimamu28a} and \REF{shimamu28b}, observing that \REF{shimamu28b} is much more acceptable than \REF{shimamu28a}.}
...
\ex \hspace{-1.5mm}*{\gll [Sono sigoto-ga]$_2$ Taroo$_1$-niyotte [  yotee-doori-ni kare$_1$-ga $t_2$ si-yoo-to ] $\{$kokoromi-rare/s-are$\}$-te-i-ta.\\
