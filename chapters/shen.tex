...
\ea Polish
	...
	\ea[]{ 
		...
		which Jan read {~} book\\
		\glt `Which book did Jan read?' (\citealt{Citko:2006} ex. 5a)
	}
	\ex[]{ 
		\label{shenatb1:b}
		...
		\glt `Which book did Maria recommend and Jan read?'  (\citealt{Citko:2006} ex. 6a)
	}
	\ex[*]{
		\label{shenbad}
		...
		\glt `Which book did Mary recommend and John read?' (\citealt{Citko:2006} ex. 7a)
	}
	\z 
\z 
...
\z 
\cite{Citko:2006} assumes that ATB moved elements are necessarily base-generated using multi-dominance. As is shown in \REF{shenhowmany1} for \REF{shenbad} and \REF{shenex4} for \REF{shenatb2}, the DP modifiers, \textit{how many} and \textit{which}, are simultaneously merged with both object nouns, and then moved to the Spec,CP position.  

\begin{multicols}{2}

\ea 
\label{shenhowmany1} 
structure of \REF{shenbad}: *\\
\footnotesize
\begin{forest}
qtree edges
	[CP
...
\largerpage[2]
The contrast between the two derivations above needs to be accounted for: \REF{shenhowmany1} is not accepted while \REF{shenex4} is OK. The relevant difference is whether the nouns in the object DPs are identical or distinct. When they are distinct in \REF{shenex4}, the sentence is OK; when they are identical in \REF{shenhowmany1}, the sentence is out. To rule out \REF{shenhowmany1} and retain \REF{shenex4}, \cite{Citko:2006} proposes a constraint on multi-dominance structures where the shared material must be maximized. The structure with less shared material is ruled out if an alternative structure with more shared material is available. Regarding the pattern at hand, the structure in \REF{shenhowmany1} where only \textit{which} is shared is compared with the alternative in \REF{shenhowmany2} where the entire object \textit{how many books} is shared with \textit{how many} moving away. Given \REF{shenhowmany2}, the structure with less shared material in \REF{shenhowmany1} is ruled out. As predicted by this constraint, the sentence in \REF{shenhowmany2} is indeed better than \REF{shenbad}.\footnote{\cite{Citko:2006} assumes that the sentence in \REF{shenhowmany2} involves the movement of \textit{books} to a higher position. This is to keep in line with the linearization constraint of sharing which states that all shared elements must be moved to a non-shared position to be linearized (see also \citealt{Gracanin-Yuksek:2007}). I do not follow this assumption that the noun \textit{books} moves in \REF{shenhowmany2} as many other linearization algorithms (e.g. \citealt{Wilder:2008, deVries:2009, Gracanin-Yuksek:2013}) can linearize shared materials in situ.} Note that the structure in \REF{shenex4} is not ruled out since the head nouns are distinct thus the whole DP cannot be shared. 
...
\ea[?]{
	...
	\glt `How many books did Maria recommended and how many books did Jan read?' (modified from \citealt{Citko:2006} p238, 26b)\\
\footnotesize
\begin{forest}
qtree edges
	[CP
...
\cite{Shen:2018a} discusses number marking on the head noun which is shared by two singular DPs as is shown in \REF{shennrnr}. 
...
\REF{shennp1} and \REF{shennp2} illustrate what the structure would look like under the intended dual student reading. 
In \REF{shennp1}, the noun is shared and the \textsc{num}P is shared in \REF{shennp2}. 
...
Similar to Citko, Shen proposes a MaxShare constraint on the size of the shared element to rule out the structures in \REF{shennp1} and \REF{shennp2}. The constraint is as seen in \REF{shenform1} where \textit{sharable} is defined as non-distinct.
...
\REF{shenform1}, the potential alternative structure in \REF{shenmax} where the Poss' is shared rules out the structures in \REF{shennp1} and \REF{shennp2}. 
As one can see, the shared constituent in \REF{shenmax}, Poss', properly contains the ones shared in \REF{shennp1} and \REF{shennp2}.\footnote{The readers will notice that \REF{shenftn5exi} is OK where the head noun is plural under possessive DPs. 
\cite{Shen:2018a} argues that \REF{shenftn5exi} involves a structure with a single DP with the conjoined possessors in its specifier position with a plural \textsc{num} head. 
...
\z
%start here
Note that although the structure in \REF{shenmax} does not violate MaxShare, it must be ruled out as well since the string `John's and Mary's student' is not acceptable under the dual student reading. \cite{Shen:2018a} proposes that the structure in \REF{shenmax} is ruled out by an independent requirement on sharing: the Agree constraint, which requires the head of the shared element (Poss in \REF{shenmax}) agrees with the remnants (\textit{John's} and \textit{Mary's}). Since there is no agreement between the possessors and the Poss' or the Poss head, the structure in \REF{shenmax} is ruled out. I will follow this analysis here (see brief discussion in Section \ref{shensect:ban}).
%
...
\ea 
	...
	\ea 
		...
		Numeration for \REF{shenhowmany1} = \{how-many$_1$, Maria$_1$, Jan$_1$, recommended$_1$, read$_1$, \textbf{books$_2$}, and, T$_2$, v$_2$, C\}
	\ex 
...
Take \REF{shenex13} for example (modified from (14) in \citealt{Gracanin-Yuksek:2007}). 
...
%
%\cite{Gracanin-Yuksek:2007} makes the distinction between bulk and non-bulk sharing. See a minimal pair in \Next. Both derivations involve the string \textit{X$_1$ and X$_2$ Y ZP}, however, in \Next[a], the YP is shared as a whole whereas in \Next[b] it is the daughter nodes of YP, namely Y and ZP, that are shared respectively. 
%
%\begin{multicols}{2}
%
%\ex.\label{shenbnnb1}
...
\ea 
	...
	\ex[*]{
		...
		\glt `What kind of coffee did Maria order and what kind of tea did Jan order?' (see (28) in \citealt{Citko:2006} for another example)
	}
	\z 
\z 
...
The structures are illustrated in \REF{shenex16} with English translation.\footnote{The shared verb is assumed to move to a higher node in \cite{Citko:2011}'s proposal. 
%Citko2006a,
Here I kept the verb at the shared position to better illustrate the fact that the verb is shared.}
In the structure for \REF{shengapAtb:a} in \REF{shenex16:a}, both the pre-nominal modifier and the verb are shared, the sentence is accepted. 
In \REF{shenex16:b} for \REF{shengapAtb2}, only one of the two shareable elements is shared, i.e.  the pre-nominal modifier \textit{which}, whereas the verb \textit{ordered} which could be shared, is not.
...
\ea 
\label{shenex16:a}
structure for \REF{shengapAtb:a}\\
\footnotesize
\begin{forest}
qtree edges
[CP
...
The numeration MaxShare correctly rules out the derivation in \REF{shenex16:b}. The numerations of \REF{shenex16:a} and \REF{shenex16:b} are shown in \REF{shennum}. They contain the same items but the verb \textit{ordered} is selected once in \REF{shennum:a} but twice in \REF{shennum:b}. Thus the numeration in \REF{shennum:b} is ruled out given the more economical numeration in \REF{shennum:a}.
On the other hand, the dominance MaxShare does not predict the contrast in \REF{shengapAtb}, since \REF{shenex16:a} does not involves sharing of a constituent that dominates the shared constituent in \REF{shenex16:b}.

\ea 
	\label{shennum}
	\ea 
		\label{shennum:a}
		Numeration for \REF{shenex16:a} = \{which$_1$, Jan$_1$, Maria$_1$, \textbf{ordered$_1$}, coffee$_1$, tea$_1$, and$_1$, T$_2$, v$_2$, C$_1$\} 
	\ex 
		\label{shennum:b}
		Numeration for \REF{shenex16:b} = \{which$_1$, Jan$_1$, Maria$_1$, \textbf{ordered$_2$}, coffee$_1$, tea$_1$, and$_1$, T$_2$, v$_2$, C$_1$\} 
	\z 
\z 
...
\ea 
	...
	\z 
\z 
One possible explanation for the degraded but accepted status of \REF{shenex18:a} and \REF{shengapAtb2}, both of which violate the numeration MaxShare, is that the numeration MaxShare is a violable constraint and does not immediately cause a derivation to crash. On the other hand, the unacceptability in NP RNR in \REF{shennrnr} (repeated here as \REF{shenex19}) is quite strong. \cite{Shen:2018a} reports experimental results using judgments on a 7 point Likert scale from 45 participants and show that \REF{shenex19} which involves a MaxShare violation has a mean rating of 2.33 out of 7. Note that \REF{shenex19} violates the dominance MaxShare (and by entailment, also the numeration MaxShare). 
...
In the implementation of the numeration MaxShare presented so far, the entire utterance including the conjunction phrase is assumed to share one numeration, as is illustrated above in \REF{shennum}. In order to account for \REF{shenex21}, we need to further break down the derivation. In the multiple spell-out model proposed by \cite{Uriagereka:1999a, Chomsky:2000} , numeration, derivation, and spell-out occur in phases. \cite{Oda:2017} proposes that the \&P and its conjuncts are phases to account for the cross-linguistic patterns of the Coordinate Structure Constraint. 
...
The claim here is that only the set of numerations that meet certain conditions are compared in terms of economy. One such condition is that these numerations must contain the same set of unique lexical items. They can, however, differ in the number of `copies' of the lexical items.\footnote{This restriction on MaxShare is by no means the only restriction. \cite{Shen:2018a} briefly discusses an interpretative restriction on MaxShare (Section 2.6.2):  the structures being compared must be of the same interpretation. The evidence is shown in \REF{shenftn8exi}. The sentence in \REF{shenftn8exi} is ambiguous between \REF{shenftn8exi:a} where \textit{tall} is not shared, and \REF{shenftn8exi:b} where \textit{tall} is shared. If MaxShare does not care about interpretations, the interpretation in \REF{shenftn8exi:a} should not be available since it involves a structure where less material is shared than in the structure that generates \REF{shenftn8exi:b}.
...
This section explores an alternative to MaxShare to account for patterns of NP RNR. Evidence for MaxShare from NP RNR comes from the unacceptability of \REF{shensvs}. \cite{Shen:2018a} argues that the singular marking on the shared noun requires multi-dominance and the unacceptability of \REF{shensvs} indicates that a multidominance structure is ruled out. As mentioned above, the account proposed in \cite{Shen:2018a} involves two constraints on multi-dominance: one is MaxShare, and the other is an Agree requirement where the shared element and the sharing elements must agree. We have seen how MaxShare rules out structures that would generate \REF{shensvs} in the discussion above. The Agree requirement rules out the structure that does not violate MaxShare. In the structure in \REF{shenmax}, the largest shareable constituent \textsc{Poss'} is shared in accordance with MaxShare, however, this structure is ruled out because there is no agreement relation between the possessors \textit{John's} and \textit{Mary's} and the shared \textsc{Poss} head.
...
\end{forest}
\z
\REF{shensvs} contrasts with the sentence in \REF{shenex25} where the head noun is plural. \cite{Shen:2018a} argues that the plural noun indicates a different structure, illustrated below: \textit{John's} and \textit{Mary's} are conjoined in the Spec,DP position. No sharing/multidominance is involved. 
...
The motivation behind the Agree requirement and MaxShare is to rule out \REF{shensvs} independently from \REF{shenex25}. However, based on the contrast between the two sentences, one can imagine an alternative where it is the availability of the structure in \REF{shenex25} that blocked the multidominance structure in \REF{shensvs}. I formulate the constraint in \REF{shenban} and refer to it as the \textsc{Ban}.
%
\ea 
	\label{shenban}
...
\z 
The idea behind \REF{shenban} is that the option of sharing can only be entertained if the string cannot be generated otherwise. From this perspective, sharing is used as a last resort operation. Let's see how the \textsc{ban} in \REF{shenban} can rule out \REF{shensvs}. In the string \textit{John's and Mary's X}, there are at least two possible parses shown in \REF{shenex27}. \REF{shenex27:a} is a parse with a shared X while \REF{shenex27:b} involves conjoined specifiers and no sharing. The \textsc{Ban} in \REF{shenban} states that the \REF{shenex27:a} is ruled out since \REF{shenex27:b} is available. Thus this constraint alone can replace both MaxShare and the Agree requirement.
...
\normalsize
The \textsc{Ban} predicts that sharing becomes available once the string cannot be generated otherwise. This prediction is supported by the phrase in \REF{shenex28}. The singular noun indicates that the head noun \textit{student} is shared. This is expected since \textit{John's tall} and \textit{Mary's short} cannot be conjoined as is shown in \REF{shenex28:a} because they do not form constituents (assuming that only constituents can be conjoined). In other words, the string cannot be generated without invoking sharing, thus sharing is available as is shown in \REF{shenex28:b}.
...
Another case where these conditions are met is in \REF{shenex29}, which also has two potential structures. \REF{shenex29:a} illustrates one where the T' is shared and \REF{shenex29:b} illustrates one that does not involve sharing but the conjunction of the subjects. The telltale sign to differentiate the two structure is the number marking on the verb. According to \cite{Kluck:2009, Grosz:2015, Shen:2019}, the structure in \REF{shenex29:a} is compatible with both the singular and the plural auxiliary whereas the conjoined subject in \REF{shenex29:b} requires the auxiliary to be plural. As is shown in \REF{shenex29}, only the plural auxiliary is available, which indicates that the sharing structure \REF{shenex29:a} is ruled out while the non-sharing structure \REF{shenex29:b} is ruled in. This is expected from the \textsc{Ban}.
...
In \REF{shenex30}, neither \textit{John always} nor \textit{Mary sometimes} form a constituent, thus conjunction structure in \REF{shenex29:b} is impossible. 
...
The next example of the \textsc{Ban} I will present here also involves NP RNR but with a different telltale sign: interpretation. The string in \REF{shenex31} can potentially be generated via a structure where \textit{dress} is shared by \textit{blue} and \textit{black} shown in \REF{shenex31:a} or via a structure where \textit{blue} and \textit{black} are conjoined as shown in \REF{shenex31:b}. The non-sharing structure with the singular noun must refer to a single dress that's both blue and black, whereas the sharing structure, also with the singular noun, must refer to two different dresses, one being blue and the other black. The absence of the two-dress reading indicates that sharing is ruled out while the non-sharing structure is available (indicated by the one-dress reading). 
...
However, this means that the Ban can not rule out the less acceptable \REF{shenbad2}. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{ATB movement + LBE}

\ex. LBE
\ag. Kt\'{o}ra$_i$ Jan przeczytał t$_i$ ksiazke? [Pol]\\
which Jan read {~} book\\
`Which book did Jan read?'
\bg. Kakva$_i$ si kupio t$_i$ kola? \hfill BCS\\
what-kind-of are bought {~} car\\
`What kind of a car did you buy'
\cg. Kiro$_i$ je Janez prebral t$_i$ knjigo? \hfill Slovenian\\
which aux Jan read {~} book\\
`Which book has John read?'

\ex. ATB 
\ag. Ktora ksiazke$_i$ Maria poleciła t$_i$ a Jan przeczytał t$_i$? [Pol]\\
which book Maria recommended {~} and Jan read {~} \\
Which book did Maria recommend and Jan read?
\bg. Kiro knjigo$_i$ je Janez priporocu t$_i$ in Marija prebrala t$_i$? \\
which book is John recommended {~} and Mary read {~} \\
`Which book did John recommend and Mary read?'\hfill Slovenian

In \Last 
\begin{itemize}
	\item shared: DP
	\item MaxShare: OK
	\item Linearlization: OK
\end{itemize}

\ex. ATB + LBE (different verb, same object)
\ag. *Ktora$_i$ Maria poleciła t$_i$ ksiazke a Jan przeczytał t$_i$ ksiazke? [Pol]\\
which Maria recommended {~} book and Jan read {~}  book\\
`Which book did Mary recommend and John read?'
\bg. *Kiro$_i$ je Janez priporocu t$_i$ knjigo in Marija prebrala t$_i$ knjigo? \hfill Slovenian\\
which aux Jan recommended {~} book and Maria read {~} book\\
`Which book did John recommend and Mary read?'

\begin{itemize}
	\item When DP is shared
	\begin{itemize}
		\item MaxShare: OK
		\item Linearization: * 
	\end{itemize}
	\item When D is shared
	\begin{itemize}
		\item MaxShare: *
		\item linearization: OK
	\end{itemize}
\end{itemize}

\ex. ATB + LBE same verb but gapped, different object
\ag. Jaka$_i$ Maria zamowiła t$_i$ kawe˛ a Jan \sout{zamowił} t$_i$ herbate? [Pol]\\
which Maria ordered {~} coffee and Jan \sout{ordered} {~}  tea \\
`What kind of coffee did Maria order and what kind of tea did Jan
order?'
\bg. Cigavo$_i$ je Janez prebral t$_i$ pesem a Marija \sout{prebrala} t$_i$ knijigo? \hfill [Slv]\\
whose aux Jan read {~} poem and Mary \sout{read }{~} book\\
`Whose poem did John read and whose book did Mary read?'

\begin{itemize}
	\item When V and D is shared
	\begin{itemize}
		\item MaxShare: OK
		\item Linearization: OK since both multi-dominated elements: V and D are moved outside the conjunction
	\end{itemize}
\end{itemize}

\ex. ATB+LBE different verb, different object
\ag. Ile$_i$ Maria napisała t$_i$ ksiazek a Jan przeczytał t$_i$ artykułow? [Pol]\\
how-many Maria wrote {~} books and Jan read {~} articles\\
`How many books did Maria write and how many articles did Jan read?'

\begin{itemize}
	\item When D is shared, because the noun is different, D is the only thing that can be shared
	\begin{itemize}
		\item MaxShare: OK
		\item linearization: OK
	\end{itemize}
...
\ex. ATB+LBE different verb, same object (same as one of the examples above)
\ag. *Ile$_i$ Maria napisała t$_i$ ksiazek a Jan przeczytał t$_i$ ksiazek? [Pol]\\
...
\ex. ATB+LBE same verb, different objects
\ag. *Ile$_i$ Maria przeczytała t$_i$ ksiazek a Jan przeczytał t$_i$ recenzji? [Pol]\\
how-many Maria read-fem {~} books and Jan read-masc {~}  reviews\\
`How many books did Maria read and how many reviews did Jan read?'


\begin{itemize}
	\item When V and D is shared, \textcolor{red}{non-bulk sharing}
	\begin{itemize}
		\item MaxShare: OK
		\item linearization: *, the verb didn't move
	\end{itemize}
	\item DP cannot be shared due to the different object
	\item When D is shared
	\begin{itemize}
		\item MaxShare: *
		\item linearization: OK
...
ATB in Polish requires everything to be distinct. 

\ex. ATB with differt subjects, same verbs
\ag. *Jaka ksiazke$_ien$ Jan przeczytał t$_i$ a Maria przeczytała t$_i$? [Pol]\\
what book Jan read {~}  and Mary read {~} \\
`What book did Jan read and Maria read?'

\begin{itemize}
	\item When V, O are shared
	\begin{itemize}
		\item MaxShare: OK
		\item Linearization: *
	\end{itemize}
	\item When O is shared
	\begin{itemize}
		\item MaxShare: *
		\item Linearization: OK
	\end{itemize}
...
\ex. FN 8, same thing in English. 
\a. ??? Which book did John read and Mary read?
...
\ex. ATB with same subjects, different verbs
\ag. *Jaka ksiazke$_i$ Jan przeczytał t$_i$ a Jan odłozył t$_i$?\\
what book Jan read {~}  and Jan filed {~} \\
`What book did Jan read and Jan file?'

\begin{itemize}
	\item When S, O are shared
	\begin{itemize}
		\item MaxShare: OK
		\item Linearization: *
	\end{itemize}
	\item When S is shared
	\begin{itemize}
		\item MaxShare: *
		\item Linearization: *
	\end{itemize}
...
\ex. ATB with different subjects, different verbs
\ag. Jaka ksiazke$_i$ Jan przeczytał t$_i$ a Maria odłozyła t$_i$?\\
what book Jan read {~} and Maria filed {~} \\
`What book did Jan read and Maria file?'

\begin{itemize}
	\item When O are shared
	\begin{itemize}
		\item MaxShare: OK
		\item Linearization: OK
	\end{itemize}
...
\ex. Gapping in English requires everything to be distinct.
\a. John likes linguistics and Mary \sout{likes} philosophy.
\b. *John likes linguistics and John \sout{likes} philosophy.
\c. *John likes linguistics and Mary \sout{likes} linguistics.

(\textcolor{red}{ZS: this can be accounted for if Gapping requires contrastive focus, right?})

Analysis
\begin{itemize}
	\item ATB is movement of multi-dominated elements
	\item Multi-dominated elements must be moved out of the conjunction to avoid linearization paradox
\end{itemize}

\begin{itemize}
	\item On derivation where both \textit{which} and \textit{book} are shared
		\begin{itemize}
			\item the stranded book will cause linearization contradiction
		\end{itemize}
	\item On the derivation where only \textit{which} is shared.
		\begin{itemize}
			\item Since only \textit{which} is shared and it's moved away, no linearization problem
			\item the derivation is ruled out because if the object N is the same, it should also be shared: MaxShare.
			\item essentially, sharing only D when you could have shared DP is bad. Implicitly, sharing D when you could have shared V and D is also bad. So it's not just a domination thing, once non-bulk sharing is allowed.
		\end{itemize}
\end{itemize}

\begin{itemize}
	\item MaxShare: an economic thing. 
	\item cross-deriational comparison of derivations with the same numeration which involve sharing of some elements
	\item (\textcolor{red}{ZS: what if it's numeration with sharing of V vs. numeration with sharing of D N})
\end{itemize}

%
%\ex. The shared determiner agrees with both DPs which cannot mismatch in gender features
%\ag. *Ktory$_i$ Maria skonczyła t$_i$ artykuł a Jan zaczał t$_i$ ksiazke? [Pol]\\
%which-sg.masc Maria finished article-sg.masc and Jan started book-sg.fem\\
%`Which article did Mary finish and which book did Jan start?'
%\bg. *Ktora$_i$ Maria skonczyła t$_i$ artykuł a Jan zaczał t$_i$ ksiazke?\\
%which-sg.masc Maria finished article-sg.masc and Jan started book-sg.fem\\
%`Which article did Mary finish and which book did Jan start?'
%
...
\ex. 
\a. John's tall and Mary's short student/*students are a couple.
\b. John's and Mary's students/*student are a couple.

\begin{itemize}
	\item Shen propose that in English the singular target noun in NP RNR involves multi-dominance.
	\item One need to explain why John's and Mary's student is ruled out.
	\item Two constraints on multi-dominance are proposed
	\begin{itemize}
		\item Agree: the target needs to agree with the sources
		\item MaxShare: the target needs to be the largest sharable constituent; i.e. smaller targets are ruled out.
	\end{itemize}
\end{itemize}

\ex. John's tall and Mary's short student are a couple.\\
Agree: OK; MaxShare: OK\\
\begin{forest}
qtree edges
	[ConjP
		[DP1
...
qtree edges
	...
	\draw (source.south) -- (numP.north);
\end{forest}


\begin{itemize}
	\item Different from Citko 2006: even the invisible heads are included in the calculation, which is expected. 
	\item  Shen's MaxShare is built on domination in bulk sharing; Citko's MaxShare is built on economy, which is in essence non-bulk sharing. It's not about larger or smaller shared constituents, it's about the number of shared terminal nodes.
	\item with non-bulk sharing, the agree requirement needs to be restated to the highest shared element needs to agree.
\end{itemize}

\section{mixing gapping and RNR}

If MaxShare is calculated based on the whole sentence, then we should be able to compare derivations involving Adjective and V.

Crucially, as is stated, it's possible for all derivations to crash. it's not the case that among otherwise grammatical derivations, the one with the largest shared element wins and the rest are ruled out. 

In other words, even the derivation that satisfies MaxShare is ruled out independently, doesn't make the derivation that violates a bit of MaxShare good. Otherwise, the DP sharing derivation of \Next would be OK.

\ex. which1 John likes t1 book and Mary hates t1 book.

Now look at \Next where gapping is involved as well as a common noun in the object DPs.

\ex. John likes this red car and Mary that green car.

\begin{itemize}
	\item According to Citko, likes is shared, but car is not; this derivation should be ruled out. 
	\item even though \Next is ruled out independently. 
\end{itemize}

\ex. John likes the red, and Mary green car.

\textcolor{red}{ZS: OK it's possible the determiner sharing depends on gapping because determiner sharing triggers MaxShare? But curiously gapping doesn't depend on determiner sharing:}

\ex. 
\a. Too many Irish setters are named Kelly, and \sout{too many} huskies \sout{are named} Nanook.
\b. Too many Irish setters are named Kelly, and too many huskies \sout{are named} Nanook.


ATB + Gapping (\textcolor{red}{ZS: not sure what these contribute})
\ex. how many did John buy cars and Mary \sout{buy} bikes?

\ex. How many John bought cars and Mary \sout{bought} bikes?

\section{NP Left Node Raising}

\ex. This man and woman\\
\begin{forest}
qtree edges
	[ConjP
		[DP1
...
qtree edges
	...
	\draw (this.north) -- (dp2.south);
	\draw (num.north) -- (nump2.south);
\end{forest}


why is this bad?
\ex. *this tall and short student are a couple\\
Agree: OK, MaxShare: ok
\begin{forest}
qtree edges
	[ConjP
	...
	\draw (D.north) -- (dp2.south);
	\draw (np1.south) -- (np.north);
	\draw (num1.north) -- (nump2.south);
\end{forest}


(\textcolor{red}{ZS: this paper can be comparing maxshare in VP/DP/CP or whole senrtencer})
%
%
%\section*{Acknowledgements}
%\citet{Nordhoff2018} is useful for compiling bibliographies

\section{Do we need MaxShare when we have contrast constraint?}
