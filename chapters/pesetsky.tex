...
Fitzpatrick then proceeds to show that an \textsc{aux}--drop question is even larger than TP, and in particular, that it is generated as a full CP, in which an auxiliary verb in T has moved to C. As he notes first, if the interpretation of examples like \noindent (\getref{auxdrop}a-d) as yes/no questions relies on semantics crucially provided by an interrogative C (as in most accounts of such questions), an \textsc{aux-}drop clause\textsc{ }must have been generated as a CP. An additional argument rests on the observation that licensing a negative polarity item like \textit{anyone} and \textit{anybody} in examples \noindent (\getref{auxdrop}a-d) correlates with movement of \textsc{aux} to C in less controversial matrix yes/no questions. For example, \textit{how come} differs from its near-synonym \textit{why} in not triggering \textsc{aux}-to-C movement, and also differs in not licensing NPIs:
...
\pex<auxinsitu>
\ptxt{\textsc{\textbf{aux}}-to-C vs. its absence correlates with NPI licensing} 
...
[T,name=tense,tikz={\node [rectangle,draw=black, fill=black, fill opacity=0.05,text opacity=1,anchor=right,ultra thick,fit=()(!u)(!uuu)(speccp)] {};}]
[\textit{to}P,name=exfoliate [\textit{\textbf{subject}},name=lowsubject] [\textit{to}$'$ [\textit{to}] [\textit{vP}]]]]]
{\draw (.east) node[right=1cm, text width=8cm] {\small{\textit{← \begin{minipage}{4.6cm}Exfoliation removes this portion of the embedded clause\end{minipage}}}}; }
]]
\begin{pgfinterruptboundingbox}
{\draw[->,thick,densely dashed] (probe) to[out=south,in=south] (lowsubject);}
...
\ptxt{Predicate fronting in an infinitival clause}
...
\pex<DATNOMR2>
\ptxt{Quirky subject + \Nom{} object in an infinitival complement (Icelandic)}\\
\begingl
...
of \noindent (\getref{pregnancy1}) may be understood as bound by
...
\a
\begingl
\gla You see John yesterday? \nogloss{\tabto{2.9in} \footnotesize{(\textsc{aux}\textit{-drop})}} //
\glft `Did you see John yesterday' //
\endgl 
\a 
\begingl 
\gla \ljudge*We believed Mary to see John yesterday. \nogloss{\tabto{2.9in}\footnotesize{(\textit{propositional infinitive})}} //
...
\noindent Independent of the puzzle of \noindent (\getref{puzzle}), the facts in \noindent (\getref{eventive3}) present an additional challenge to an Exfoliation approach to non-finite clauses, since they display another unexpected difference in the semantics of finite clauses and their infinitival counterparts. I suggest that solving the puzzle of \noindent (\getref{eventive3}) may help solve the puzzle of \noindent (\getref{puzzle}) as well.
...
Returning now to the puzzle in \noindent (\getref{puzzle}), we might explain the unavailability of a \textsc{past }interpretation for a propositional infinitive as the result of the hearer's strategy in (\getref{assumptions}):
